...
def make_marker(frame_id, id, pose, scale, color):
    msg = Marker()
...
def make_table_markers():
    ...
    markers.append(
        make_marker(
            "table_frame",
            2,
            Pose(Point(leg_xy_offset(), leg_xy_offset(), 0.67 / 2), Quaternion(w=1.0)),
            Vector3(0.03, 0.03, 0.67),
            dark_gray(),
        )
    )
...
def table_marker_publisher():
    ...
    pub_line_min_dist = rospy.Publisher(
        "test_visualization_marker", Marker, queue_size=1
    )

    rospy.sleep(0.3)
    if not rospy.is_shutdown():
        pub_line_min_dist.publish(markers)
