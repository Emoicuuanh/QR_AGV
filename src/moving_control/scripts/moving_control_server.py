...
class MovingControl(object):
    ...
    def __init__(self, name, *args, **kwargs):
        ...
        self.reset_velocity_pub = rospy.Publisher(
            "/reset_velocity", EmptyStamped, queue_size=1
        )
    ...
    def moving_control_exec_cb(self, goal):
        ...
        while goal_accept and not rospy.is_shutdown():
            ...
            if self._as.is_preempt_requested() or self._asm.reset_action_req:
                ...
                self.reset_velocity_pub.publish(EmptyStamped())
                success = False
                self.send_feedback(self._as, "PREEMPTED")
                # Call planner to reset velocity / avoid 
                break
                        ...
                        if (
                            ...
                            for i in range(
                                ...
                                if round(waypoint_pos["x"], 3) == round(
                                    ...
                                    index_to_keep = i
                                    rospy.logerr(
                                        f"find new data_dict at index {i}"
                                    )

                                    # Kiểm tra thêm 3 phần tử tiếp theo (theo hướng duyệt lùi)
                                    for j in range(1, extra_checks + 1):
                                        next_index = i - j
                                        if next_index < 0:
                                            break
                                        ...
                                        wp_next_pos = waypoints[next_index][
                                            "position"
                                        ]["position"]
                                        if round(wp_next_pos["x"], 3) == round(
                                            target_position["x"], 3
                                        ) and round(
                                            wp_next_pos["y"], 3
                                        ) == round(
                                            ...
                                            index_to_keep = next_index
                                            rospy.logerr(
                                                f"Refind new data_dict at index {next_index}"
                                            )

                                    break  # dừng sau khi kiểm tra thêm
