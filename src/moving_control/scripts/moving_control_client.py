...
class MovingControlClient(object):
    def __init__(self, json_file):
        ...
        self.moving_control_client.wait_for_server(timeout=rospy.Duration(2))
        ...
        # Action
        self.moving_control_client.send_goal(goal, feedback_cb=self.feedback)
        result = self.moving_control_client.wait_for_result()
    ...
    def feedback(self, fb):
        if fb == self.last_fb:
            ...
            rospy.loginfo(
                "Action feedback: {} ({})".format(fb, self.duplicate_count)
            )
        else:
            self.duplicate_count = 1
...
def main():
    rospy.init_node("moving_control_client")
    rospy.loginfo("Init node " + rospy.get_name())
    opt = parse_opt()
    print(opt)
    MovingControlClient(**vars(opt))
